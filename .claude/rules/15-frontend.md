...
**フロントエンドファイル（`resources/js/` 配下の `.vue` / `.ts` / `.js`、`resources/css/`）を編集した後は、テストやタスク完了報告の前に必ずプロジェクトのフロントエンドビルドコマンド（例: `npm run build`）を実行すること。** Viteはソースファイルをハッシュ化されたバンドルとして `public/build/` にコンパイルする——ブラウザは常にコンパイル済みバンドルを読み込み、生のソースは読み込まない。そのためビルドせずに編集すると古いバンドルが動き続け、変更が反映されていないように見える。

- ビルドは**タスクごとに1回**、全フロントエンドファイルの編集が完了した後に実行する（1ファイル保存するたびに実行しない）
- バックエンドのみの変更（`app/**/*.php`、マイグレーション、`.env`）はビルド不要
- ユーザーから「フロントエンドの変更が反映されていない」と報告された場合は、まずビルド漏れを疑う
- ビルドステップを持たない別のフロントエンドスタック（素のBlade/Livewire等）を選定した場合、このファイルをそのスタック用に書き換える際に本セクションを削除する
