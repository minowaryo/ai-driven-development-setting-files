...
> **プレースホルダー確認**: このファイルがまだ `[PROJECT_NAME]` のようなプレースホルダーのままで、
> かつリポジトリに既に実質的なアプリケーションコード（models/controllers/migrations等、雛形以上のもの）
> が存在する場合は、他の依頼への着手前に一旦止まり、既存コードベースへの導入の可能性があることをユーザーに
> 伝えること。`SETUP.md` の既存コードベース導入パスを案内し、`/onboard-existing-codebase` の実行を提案する。
